<template>
    <div class="container" >
        <div class="card" v-for="(repo,i) in repos" :key="{i}" >
            <div class="row">
                <div class="col-3">
...
export default {
    ...
    methods : {
        getRepos () {
            console.log("---------------", this.repository, this.language)
            axios.get(`http://localhost:4000/index/search?repository=${this.repository}&languange=${this.language}`,)
            .then(  data  => {
                console.log(data.data)
                this.repos = data.data.items
            })
            .catch(err => {
                alert(err.message)
            })
